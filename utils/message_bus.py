...
class MqttBus(MessageBusInterface):
    ...
    def connect(self) -> None:
        ...
        # If client was cleared, re-initialize it
        if not self._client:
            self._init_client()
    ...
    def disconnect(self) -> None:
        ...
        # Explicitly release the client object
        self._client = None
    ...
    def _on_disconnect(self, client, userdata, flags, rc, properties=None):
        self.logger.info(f"MQTT client disconnected with code {rc}")
        self._set_state(BusConnectionState.DISCONNECTED)

        # If the disconnection was not intentional, start the reconnect process.
        # A clean disconnect typically has rc == 0.
        if rc != 0:
            self.logger.warning(
                f"Unexpected MQTT disconnection (rc={rc}). "
